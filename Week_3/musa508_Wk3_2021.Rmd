...
ggplot()+
  ...
  geom_sf(data = uga, fill = "transparent", color = "black")+
  mapTheme()
```
...
```{r buffer_plot}
ggplot() + 
  geom_sf(data = bothBuffers, aes(fill = Legend),lwd = 0) +
  scale_fill_manual(values = c("#66DDDD", "#00CCFF")) + 
...
ggplot() + 
  ...
  geom_sf_text(data = buffersAndTowns %>%  dplyr::filter(Legend == 'Inside'), aes(label = MUNI), size = 2) +
  labs(title = "1/8mi buffer inside & outside UGA") +
  mapTheme()
```
...
ggplot() + 
  geom_sf(data = uga)+
  geom_sf(data = buffersAndTowns%>%dplyr::filter(Legend == 'Inside'), aes(fill = MUNI)) +
  geom_sf(data = buffersAndTowns%>%dplyr::filter(Legend == 'Outside'), aes(fill = MUNI),alpha=0.5) +
  geom_sf_text(data = buffersAndTowns %>%  group_by(MUNI)%>% summarise(), aes(label = MUNI), size = 2) +
  labs(title = "1/8mi buffer inside & outside UGA") +
  mapTheme()
```
