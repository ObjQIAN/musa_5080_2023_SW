...
# Retrive narcotics data from 2017
Narcotics <- 
...
ggplot() +
  ...
  geom_sf(data = Narcotics, colour="pink", size=0.1, show.legend = "point") +
...
ggplot() +
  geom_sf(data = Narcotic_net, aes(fill = countNarcotics), color = NA) +
...
```
```{r}
ggplot(data = Narcotic_net) +
  geom_histogram(aes(x = countNarcotics), bins=30, fill="blue", color="black") + 
  labs(title="Histogram of Narcotics Count", x="Narcotics Count", y="Number of Grid Cells") #+
  #theme_minimal()
```
...
Economic Deprivation: Neighborhoods with higher poverty rates, unemployment, or low educational attainment might have higher drug crime rates.

Population Density: Denser areas might have more person-to-person interactions, leading to more opportunities for drug transactions.

Proximity to Transportation Hubs: Major transportation hubs or routes can be strategic points for narcotics distribution.

Nightlife and Entertainment Districts: Areas with a high concentration of nightclubs, bars, and entertainment venues might have higher drug activity.

Past Narcotics Incidents: Historical data on narcotics-related arrests or incidents can be a predictor of future incidents.
...
Proximity to Schools: Drug-free zones around schools can lead to higher arrest rates near these zones.
```{r}
public_school <- 
...
Proximity to Treatment Centers: While treatment centers aim to help individuals with substance use disorders, the areas around them might experience increased drug-related activities.
...
#    na.omit() %>%
    ...
    st_as_sf() %>%
    st_transform(st_crs(fishnet)) %>%
    mutate(Legend = "Treatment")

ggplot()+
    geom_sf(data=fishnet) +
    geom_sf(data=Treatment) 
...
Public Housing: The density and socio-economic factors associated with public housing might influence drug activity.
```{r}
public_housing <- 
...
ggplot()+
    geom_sf(data=fishnet) +
    geom_sf(data=public_housing) 
```

Visibility and Natural Surveillance: Areas with poor lighting, obstructed views, or lack of 'eyes on the street' might be more conducive to drug transactions.
```{r}
light_off <- 
