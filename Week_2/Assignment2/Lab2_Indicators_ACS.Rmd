...
### Use `get_acs()` to get 2000 ACS data
...
D <- 
  ...
  scale_fill_manual(values = palette5,
                    labels = qBr(totalPop00, "value"),
                    name = "Popluation\n(Quintile Breaks)") +
  labs(title = "Total Population", subtitle = "DC; 2000") +
  mapTheme() + 
  theme(plot.title = element_text(size=22))
```
...
Here we will modify the 2000 ACS by using `spread()` to widen the data
...
Tracts 2000 is now complete. Let's grab 2020 tracts and do the same
thing.
...
To combine the 2000 and 2020 data into the same data frame we use
...
Let's bring in DC's two main subway lines as spatial data and then `rbind` them together into one object, where each row is a stop, and the columns represent the line and the name of the stop. So we select only the `Station` and `Line` information (see how we do this in the rbind!), and then `st_transform` the whole thing to be the same crs as `tracts00`
...
Create an sf object with ONLY the unioned buffer from the `wmBuffers`
spatial data frame.
...
tracts from `tracts00` that intersect the buffer of transit stop in
...
-   The intersection returns only the areas where both `buffer` and
    `tracts00` polygons overlap. This is often referred to as "clipping"

-   The intersection polygons are returns the full extend of any polygon
    in `tracts00` that touch (or intersect) the `buffer`. There is no
...
-   The intersection of polygons to centroids returns the full extent of
    any polygon in `tracts00` that has its center (called a centroid)
    within the `buffer`
...
1.  `selection1` uses `tracts00[buffer,]` uses the least code, but is
    often confusing unless you know the {sf} package well. In this
    usage, the intersecting rows of `tracts00` are selected for by
...
2.  `selection2` is a longhand way to do the same. This uses
    `tracts00[st_intersects(tracts00, buffer) %>% lengths > 0, ]` to
    spell out all of the steps used in the first method. This can be
    read as: Select all rows of the rows from `tracts00` where
...
Finally, we are using centroids, or the point at the center of a polygon, to select whether a polygon from `tracts00` intersects the `buffer`. The steps here are:
...
3.  join the original `tracts00` data frame using the `GEOID` as the join field. Note that only the `GEOID` column is selected, however the polygon spatial features are also joined.
...
that this contains a correct 2000-2020 inflation calculation
...
centerCity <-
  st_intersection(
    st_buffer(filter(wmStops, Line == "El"), 2640) %>% st_union(),
    st_buffer(filter(wmStops, Line == "Broad_St"), 2640) %>% st_union()) %>%
  st_sf() %>%
...
septa_MRB <- multipleRingBuffer(st_union(wmStops), 47520, 2640)
